...
def inspect_atten_map_by_locations(
    ...
    feature_hooks = [
        model.module.l0.register_forward_hook(
            lambda self, input, output1: features.append(output1)
        )
    ]

    atten_maps_hooks = [
        model.module.backbone.register_forward_hook(
            lambda self, input, output: global_enc_atten_maps.append(output)
        )
        for i in range(len(model.module.backbone))
    ]
